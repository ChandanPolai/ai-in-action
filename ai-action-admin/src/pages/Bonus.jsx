...
import { Plus, Pencil, Trash2, Shield, Upload, Gift, Search } from 'lucide-react';
...
import {
  ...
  deleteBonusThunk,
  setBonusAccessThunk,
  fetchBonusAccessMatrixThunk
} from '../store/slices/bonusesSlice';
...
const BonusPage = () => {
  ...
  const { list, loading, accessMatrix } = useSelector((state) => state.bonuses);
  const [modalOpen, setModalOpen] = useState(false);
  const [accessOpen, setAccessOpen] = useState(false);
  const [editing, setEditing] = useState(null);
  const [accessBonus, setAccessBonus] = useState(null);
  ...
  const openAccess = async (b) => {
    setAccessBonus(b);
    setUserSearch('');
    setAccessOpen(true);
    const result = await dispatch(fetchBonusAccessMatrixThunk(b.id));
    if (fetchBonusAccessMatrixThunk.fulfilled.match(result)) {
  ...
  const buildPayload = () => {
    ...
    fd.append('amount', form.amount === '' ? '0' : String(form.amount));
    fd.append('isActive', String(form.isActive));
    if (form.imageFile) fd.append('image', form.imageFile);
    if (editing) fd.append('bonusId', editing.id);
    return fd;
  };
  ...
  const handleSave = async (e) => {
    e.preventDefault();
    if (!form.title.trim()) {
      toast.error('Bonus title is required');
      return;
    }
    ...
    try {
      ...
      if (editing) {
        await dispatch(updateBonusThunk(payload)).unwrap();
        toast.success('Bonus updated');
      } else {
        await dispatch(createBonusThunk(payload)).unwrap();
  ...
  const handleDelete = async (b) => {
    if (!window.confirm(`Delete bonus "${b.title}"?`)) return;
    try {
      await dispatch(deleteBonusThunk(b.id)).unwrap();
  ...
  const handleToggleActive = async (b, makeActive) => {
    if ((b.isActive !== false) === makeActive) return;
    try {
      await postRequest('/admin/bonuses/toggle-status', {
        bonusId: b.id,
        isActive: makeActive
      });
      toast.success(makeActive ? 'Bonus activated' : 'Bonus deactivated');
      dispatch(fetchBonusesThunk({}));
    } catch (err) {
  ...
  const saveAccess = async () => {
    ...
    try {
      await dispatch(
        setBonusAccessThunk({
          bonusId: accessBonus.id,
          assignedUsers: selectedUsers,
          mode: 'replace'
        })
      ).unwrap();
      toast.success('Bonus assigned to selected users');
      setAccessOpen(false);
      dispatch(fetchBonusesThunk({}));
    } catch (err) {
      toast.error(err);
    ...
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Create bonuses with title, description, amount and image, then assign to users.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        ...
        {!loading && list.length === 0 && (
          <p className="col-span-full text-center py-10 text-slate-400">No bonuses yet</p>
        )}
        {list.map((b) => (
          <Card key={b.id} className={`!p-0 overflow-hidden ${b.isActive === false ? 'opacity-75' : ''}`}>
            ...
            <div className="p-4 space-y-2">
              <div className="flex items-start justify-between gap-2">
                <p className="font-bold text-slate-800 line-clamp-2">{b.title}</p>
                <Badge variant={b.isActive !== false ? 'success' : 'default'}>
                  {b.isActive !== false ? 'Active' : 'Hidden'}
                </Badge>
              </div>
              <p className="text-sm text-slate-500 line-clamp-2">{b.description || 'No description'}</p>
              <div className="flex flex-wrap gap-2 text-xs text-slate-500">
                ...
                <span className="px-2 py-1 rounded-lg bg-slate-50">{b.assignedCount || 0} users</span>
              </div>
              <div className="flex items-center gap-1 border-t border-slate-100 pt-3">
                <button
                  type="button"
                  onClick={() => openAccess(b)}
                ...
                >
                  <Shield className="w-3.5 h-3.5" /> Assign
                </button>
                <div className="inline-flex rounded-lg border border-slate-200 overflow-hidden ml-1">
                  <button
                    type="button"
                    onClick={() => handleToggleActive(b, true)}
                    className={`px-2 py-1 text-[11px] font-semibold ${
                      b.isActive !== false ? 'bg-emerald-500 text-white' : 'bg-white text-slate-500'
                    }`}
                  >
                    On
                  </button>
                  <button
                    type="button"
                    onClick={() => handleToggleActive(b, false)}
                    className={`px-2 py-1 text-[11px] font-semibold border-l border-slate-200 ${
                      b.isActive === false ? 'bg-rose-500 text-white' : 'bg-white text-slate-500'
                    }`}
                  >
                    Off
                  </button>
      ...
      <Drawer
        ...
        title={`Assign — ${accessBonus?.title || ''}`}
        size="lg"
      >
        <p className="text-sm text-slate-500 mb-3">
          Selected users will see this bonus in their panel.
        </p>
        <div className="mb-3">
          <Input
            icon={Search}
        ...
        <div className="max-h-[55vh] overflow-y-auto border border-slate-200 rounded-xl p-3 space-y-2 mb-4">
          ...
          {filteredAccessUsers.map((u) => (
            <label key={u.id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-50 cursor-pointer">
              ...
              {u.isAllowed && <Badge variant="info">Assigned</Badge>}
            </label>
          ))}
        </div>
